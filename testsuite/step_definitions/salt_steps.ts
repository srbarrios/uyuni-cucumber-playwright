...
Then(/^the download should get a (\d+) response$/, async function (code) {
    // Not possible to port this without a download tool.
    throw new Error('This step cannot be ported without a download tool.');
});

Then(/^the download should get no error$/, async function () {
    // Not possible to port this without a download tool.
    throw new Error('This step cannot be ported without a download tool.');
});
