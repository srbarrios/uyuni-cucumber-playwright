...
import {fileExtract, fileInject, getBrowserInstances, getCobblerTest, getTarget} from '../helpers/index.js';
...
When(/^I run Cobbler buildiso "([^"]*)" for distro "([^"]*)"$/, async function (param, distro) {
    ...
    const server = await getTarget('server');
    await this.runStep(`I run Cobbler buildiso for distro "${distro}" and all profiles`);
