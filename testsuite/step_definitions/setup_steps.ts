...
import {waitUntilOnboardingCompleted} from '../helpers/setup_helper.js';
...
When(/^I enter the address of the HTTP proxy as "([^"]*)"$/, async function (hostnameField: string) {
    ...
    const byLabel = getCurrentPage().getByLabel(hostnameField);
    if (await byLabel.isVisible()) {
        await byLabel.fill(value);
    } else {
        const input = getCurrentPage().locator(`#${hostnameField}, [name="${hostnameField}"]`).first();
...
When(/^I enter the SCC credentials$/, async function () {
    ...
    const userField = getCurrentPage().locator('#edit-user, [name="edit-user"]').first();
    const passField = getCurrentPage().locator('#edit-password, [name="edit-password"]').first();
...
When(/^I wait until the SCC credentials are valid$/, async function () {
    ...
    const userPanel = getCurrentPage().getByRole('region', {name: creds.username});
...
Then(/^the credentials for "([^"]*)" should be invalid$/, async function (user) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const userPanel = getCurrentPage().getByRole('region', {name: user});
...
When(/^I (deselect|select) "([^"]*)" as a product$/, async function (action, product) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const productWrapper = getCurrentPage().locator('.product-details-wrapper', {hasText: product});
...
When(/^I select or deselect "([^"]*)" beta client tools$/, async function (channel) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const productWrapper = getCurrentPage().locator('.product-details-wrapper', {hasText: channel});
    const checkbox = productWrapper.getByRole('checkbox').first();
...
When(/^I wait at most (\d+) seconds until the tree item "([^"]+)" has no sub-list$/, async function (timeout, item) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const productWrapper = getCurrentPage().locator('.product-details-wrapper', {hasText: item});
...
When(/^I wait at most (\d+) seconds until the tree item "([^"]+)" contains "([^"]+)" text$/, async function (timeout, item, text) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const itemPanel = getCurrentPage().locator('.product-details-wrapper', {hasText: item});
...
When(/^I select the addon "(.*?)"$/, async function (addon) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const productWrapper = getCurrentPage().locator('.product-details-wrapper', {hasText: addon});
    const checkbox = productWrapper.getByRole('checkbox').first();
    await checkbox.check();
});
...
When(/^I click the Add Product button$/, async function () {
    if (!getCurrentPage()) throw new Error('No page instance');
    const addButton = getCurrentPage().getByRole('button', {name: 'Add Product'});
...
When(/^I wait at most (\d+) seconds until I see the name of "([^"]*)", refreshing the page$/, async function (seconds, host) {
    ...
    await repeatUntilTimeout(async () => {
        ...
        if (await getCurrentPage().getByText(systemName, {exact: false}).isVisible({timeout: 3000})) {
            return true;
        }
...
When(/^I wait at most (\d+) seconds until onboarding is completed for "([^"]*)"$/, async function (seconds, host) {
    await waitUntilOnboardingCompleted(Number(seconds), host);
});

When(/^I wait until onboarding is completed for "([^"]*)"$/, async function (host) {
    await waitUntilOnboardingCompleted(TIMEOUTS.long, host);
});
...
Then(/^I should see "([^"]*)" at least (\d+) minutes after I scheduled an action$/, async function (text, minutes) {
    ...
    const elements = await getCurrentPage().locator('div', {hasText: text}).all();
...
Then(/^I should see the toggler "([^"]*)"$/, async function (targetStatus) {
    ...
    switch (targetStatus) {
        case 'enabled':
            toggler = getCurrentPage().locator('i.fa-toggle-on');
            break;
        case 'disabled':
            toggler = getCurrentPage().locator('i.fa-toggle-off');
            break;
        default:
...
Then(/^I should see "([^"]*)" "([^"]*)" for the "([^"]*)" channel$/, async function (targetRadio, targetStatus, targetChannel) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const link = getCurrentPage().locator(`a:has-text("${targetChannel}")`);
    const channelId = await link.getAttribute('href').then(href => href?.split('?')[1].split('=')[1]);
    let radio;
    switch (targetRadio) {
        case 'No change':
            radio = getCurrentPage().locator(`input[type='radio'][name='ch_action_${channelId}'][value='NO_CHANGE']`);
            break;
        case 'Subscribe':
            radio = getCurrentPage().locator(`input[type='radio'][name='ch_action_${channelId}'][value='SUBSCRIBE']`);
            break;
        case 'Unsubscribe':
            radio = getCurrentPage().locator(`input[type='radio'][name='ch_action_${channelId}'][value='UNSUBSCRIBE']`);
            break;
        default:
...
When(/^I wait until radio button "([^"]*)" is checked, refreshing the page$/, async function (arg1) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const radio = getCurrentPage().locator(`input[type="radio"][id="${arg1}"]`);
    await expect(radio).toBeChecked();
});

When(/^I wait until "([^"]*)" has been checked$/, async function (text) {
    if (!getCurrentPage()) throw new Error('No page instance');
    const checkbox = getCurrentPage().getByLabel(text);
    await expect(checkbox).toBeChecked();
});
...
When(/^I check for failed events on history event page$/, async function () {
    if (!getCurrentPage()) throw new Error('No page instance');
    await followLinkInContentArea('Events');
    await followLinkInContentArea('History');
    await shouldSeeText('System History');
    const rows = await getCurrentPage().locator('xpath=//div[@class=\'table-responsive\']/table/tbody/tr').all();
    let failures = '';
