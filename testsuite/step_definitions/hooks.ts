import {
    Before, After, AfterAll, BeforeAll, Given, When, Then, World, setWorldConstructor, setDefaultTimeout, Status,
    AfterStep
...
import {
    ...
    getSccCredentials,
    SccCredentials,
    getCurrentPage, GLOBAL_VARS, getContext, addContext, logScenarioTiming, setFeatureScope
} from '../helpers/index.js';
import { getProduct, suseProxyTransactional, suseProxyNonTransactional } from '../helpers/core/commonlib.js';
...
import { RemoteNode } from '../helpers/system/remote_node.js';
...
async function webSessionIsActive(): Promise<boolean> {
...
async function clickDetailsIfPresent(): Promise<void> {
  ...
  const hasDetails = await getCurrentPage().locator('text=Details').isVisible({ timeout: 0 });
...
async function printServerLogs(): Promise<void> {
  ...
  let {stdout: stdout1} = await (await getTarget('server')).run('tail -n20 /var/log/rhn/rhn_web_ui.log | awk -v limit="$(date --date="5 minutes ago" "+%Y-%m-%d %H:%M:%S")" " $0 > limit"', { checkErrors: false });
  stdout1.split('\n').forEach(line => console.log(line));
  console.log('');

  console.log('=> /var/log/rhn/rhn_web_api.log');
  let {stdout: stdout2} = await (await getTarget('server')).run('tail -n20 /var/log/rhn/rhn_web_api.log | awk -v limit="$(date --date="5 minutes ago" "+%Y-%m-%d %H:%M:%S")" " $0 > limit"', { checkErrors: false });
  stdout2.split('\n').forEach(line => console.log(line));
  console.log('');
}
...
Before('@pxeboot_minion', async function () {
  skipThisScenarioUnless(this.pxebootMac);
});
...
Before('@sle12sp5_terminal', async function () {
  skipThisScenarioUnless(this.sle12sp5TerminalMac);
});
...
Before('@sle15sp4_terminal', async function () {
  skipThisScenarioUnless(this.sle15sp4TerminalMac);
});
...
Before('@salt_bundle', async function () {
  skipThisScenarioUnless(this.useSaltBundle);
});

// do some tests only if we are using salt bundle
Before('@skip_if_salt_bundle', async function () {
  if (this.useSaltBundle) {
    skipThisScenario();
  }
});
...
Before('@transactional_server', async function () {
  skipThisScenarioUnless(this.isTransactionalServer);
});
...
Before('@beta', async function () {
  skipThisScenarioUnless(this.betaEnabled);
});
