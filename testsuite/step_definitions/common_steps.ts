...
Then(
    ...
    async function (host: string) {
        ...
        await expect(page.getByText(node.hostname)).toBeVisible();
    }
);
...
Then(
    ...
    async function (host: string) {
        ...
        await expect(page.getByText(stdout.trim())).toBeVisible();
    }
);
...
Then(
    ...
    async function (host: string) {
        ...
        if (osFamily.includes('sles')) {
            await expect(page.getByText(osVersion.replace('-SP', ' SP'))).toBeVisible();
        }
    }
);
...
Then(
    ...
    async function (host: string) {
        ...
        await expect(page.getByText(ipv4_address)).toBeVisible();
    }
);
...
Then(
    ...
    async function (host: string) {
        ...
        const uptimeLocator = page.getByRole('cell', {name: 'Last Booted'}).locator('following-sibling::td/time');
...
When(
    ...
    async function (pickup_timeout, complete_timeout, event) {
        ...
        await followPending();
        await waitUntilSeeText('Pending Events');
        await waitUntilDoNotSeeTextRefreshingPage(event, parseInt(pickup_timeout, 10));
        await followHistory();
        ...
        await waitUntilEventIsCompletedRefreshingPage(parseInt(complete_timeout, 10));
    }
);
...
When(/^I enter as remote command this script in$/, async function (multiline) {
    const {page} = getBrowserInstances();
    await page.getByLabel('Script').fill(multiline);
});
...
When(/^I check the ram value of the "([^"]*)"$/, async function (host: string) {
    ...
    await expect(page.getByText(ram_mb)).toBeVisible();
});
...
When(
    ...
    async function (host: string) {
        ...
        await expect(page.getByText(mac_address)).toBeVisible();
    }
);
...
Then(
    ...
    async function (host: string) {
        ...
        await expect(page.getByText(`${Math.floor(cpu_mhz / 1000)} GHz`)).toBeVisible();
    }
);
...
When(/^I check radio button "(.*?)"$/, async function (radio_button: string) {
    const {page} = getBrowserInstances();
    const radio = page.locator(`input[type="radio"][name="${radio_button}"]`);
