...
import {getApiTest, getCurrentPage, globalVars, repeatUntilTimeout, retrieveBuildHostId} from '../helpers/index.js';
...
When(/^I wait at most (\d+) seconds until all "([^"]*)" container images are built correctly on the Image List page$/, async function (timeout, count) {
    await repeatUntilTimeout(async () => {
        ...
        if (await getCurrentPage().locator('xpath=//tr[td[text()=\'Container Image\']][td//*[contains(@title, \'Failed\')]]').count() > 0) {
            throw new Error('error detected while building images');
        }
        if (await getCurrentPage().locator('xpath=//tr[td[text()=\'Container Image\']][td//*[contains(@title, \'Built\')]]').count() >= Number(count)) {
            return true;
        }
        await new Promise(r => setTimeout(r, 5000));
        return false;
