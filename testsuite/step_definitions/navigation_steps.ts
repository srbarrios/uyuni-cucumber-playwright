...
When(/^I check "([^"]*)"$/, async function (identifier) {
    const checkbox = getCurrentPage().locator(`#${identifier}`);
...
When(/^I enter "([^"]*)" in the placeholder "([^"]*)"$/, async function (text, placeholder) {
    await getCurrentPage().locator(`input[placeholder="${placeholder}"]`).fill(text);
});
...
When(/^I click on "([^"]*)" in element "([^"]*)"$/, async function (text, elementId) {
    await clickButtonAndWait(getCurrentPage(), `div#${elementId}, div.${elementId}`);
});
...
When(/^I follow "([^"]*)"$/, async function (text) {
    await clickLinkAndWait(getCurrentPage(), text);
});
...
When(/^I follow "([^"]*)" in the (.+)$/, async function (text, context) {
    ...
    await clickLinkAndWait(getCurrentPage(), contextLocator, text);
});
...
Then(/^I should see "([^"]*)" as checked$/, async function (identifier) {
    ...
    const locatorById = getCurrentPage().locator(`#${identifier}`);
...
Then(/^the "([^"]*)" field should be disabled$/, async function (identifier) {
    await expect(getCurrentPage().locator(`#${identifier}`)).toBeDisabled();
});
