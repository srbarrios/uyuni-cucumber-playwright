...
export class RemoteNode {
    ...
    async initialize(): Promise<void> {
        ...
        if (this.host === 'server') {
            ...
            const {returnCode: uyuniCode} = await this.run('rpm -q patterns-uyuni_server', {checkErrors: false});
            if (uyuniCode === 0) globalVars.product = 'Uyuni';
        } else {
    ...
    async inject(testRunnerFile: string, remoteNodeFile: string): Promise<boolean> {
        try {
            if (this.hasMgrctl) {
                ...
                expect(result.returnCode, `Failed to copy ${tmpFile} to container`).toEqual(0);
            } else {
    ...
    async extract(remoteNodeFile: string, testRunnerFile: string): Promise<boolean> {
        try {
            if (this.hasMgrctl) {
                ...
                expect(result.returnCode, `Failed to extract ${remoteNodeFile} from container`).toEqual(0);
     ...
     */
    async fileDelete(file: string): Promise<boolean> {
        let cmd: string;
        if (this.hasMgrctl) {
     ...
     */
    async folderDelete(folder: string): Promise<boolean> {
        let cmd: string;
        if (this.hasMgrctl) {
        ...
        if (this.localOsFamily === 'macOS') {
            ...
            for (const dev of ['br0', 'eth0', 'eth1', 'ens0', 'ens1', 'ens2', 'ens3', 'ens4', 'ens5', 'ens6', 'ens7']) {
                const {
                    stdout,
                    returnCode
                } = await this.runLocal(`ip address show dev ${dev} | grep 'inet '`, {checkErrors: false});
                if (returnCode === 0 && stdout.trim()) {
