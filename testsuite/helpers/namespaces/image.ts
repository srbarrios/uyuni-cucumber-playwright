...
export class NamespaceImage {
  ...
  get_details(imageId: number | string): Promise<any> {
    return this.test.call('image.getDetails', { sessionKey: this.test.currentToken || '', imageId });
  }

  schedule_image_build(profileLabel: string, version: string, buildHostId: number | string, date: string): Promise<any> {
    return this.test.call('image.scheduleImageBuild', { sessionKey: this.test.currentToken || '', profileLabel, version, buildHostId, earliestOccurrence: date });
  }

  list_images(): Promise<any[]> {
...
export class NamespaceImageProfile {
  ...
  create(label: string, type: string, storeLabel: string, path: string, activationKey: string): Promise<any> {
    return this.test.call('image.profile.create', { sessionKey: this.test.currentToken || '', label, type, storeLabel, path, activationKey });
  }
  ...
  set_custom_values(label: string, values: Record<string, any>): Promise<any> {
    return this.test.call('image.profile.setCustomValues', { sessionKey: this.test.currentToken || '', label, values });
  }

  delete_custom_values(label: string, keys: string[]): Promise<any> {
    return this.test.call('image.profile.deleteCustomValues', { sessionKey: this.test.currentToken || '', label, keys });
  }

  get_custom_values(label: string): Promise<any> {
    return this.test.call('image.profile.getCustomValues', { sessionKey: this.test.currentToken || '', label });
  }

  list_image_profile_types(): Promise<any[]> {
    return this.test.call('image.profile.listImageProfileTypes', { sessionKey: this.test.currentToken || '' });
  }

  list_image_profiles(): Promise<any[]> {
    return this.test.call('image.profile.listImageProfiles', { sessionKey: this.test.currentToken || '' });
  }

  get_details(label: string): Promise<any> {
    return this.test.call('image.profile.getDetails', { sessionKey: this.test.currentToken || '', label });
  }

  set_details(label: string, details: any): Promise<any> {
...
export class NamespaceImageStore {
  ...
  create(label: string, uri: string, storeType: string, credentials: any = {}): Promise<any> {
    return this.test.call('image.store.create', { sessionKey: this.test.currentToken || '', label, uri, storeType, credentials });
  }
