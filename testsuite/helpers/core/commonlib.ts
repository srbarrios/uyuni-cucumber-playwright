 ...
 * @param page The Playwright page object
 * @param text The selector for the button
...
export async function clickButtonAndWait(
    ...
    const locator = getCurrentPage().getByRole('button', {name: text});
...
export async function clickLinkAndWait(
    page: Page,
    // TODO: We need to refactor this converted code, before we where not passing selector+text
    selector?: Locator,
    text?: string,
    options: Parameters<Page['click']>[1] = {}
): Promise<void> {
    if (text != null) {
        await getCurrentPage().click(text, options);
    }
