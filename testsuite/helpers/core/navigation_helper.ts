...
import {getCurrentPage, TIMEOUTS} from './env.js';
...
export async function toggleCheckboxInPackageList(
    ...
    if (!lastVersion) {
        return toggleCheckboxInList(getCurrentPage(), action, text);
    }

    try {
        const linkElements = await getCurrentPage().locator("div.table-responsive table tbody tr td.sortedCol a").all();
        ...
        const checkbox = getCurrentPage().locator(xpath).first();
...
export async function toggleCheckboxInList(
    ...
    const checkbox = getCurrentPage().locator(xpath).first();
    await checkbox.waitFor({timeout: TIMEOUTS.long * 1000});
...
export async function filterByPackageName(page: Page, packageName: string): Promise<void> {
    ...
    const filterInput = getCurrentPage().locator("input[placeholder='Filter by Package Name: ']");
...
export async function toggleCheckbox(page: Page, action: 'check' | 'uncheck', id: string): Promise<void> {
    const checkbox = getCurrentPage().locator(`#${id}, id=${id}, [name="${id}"]`);
    const shouldCheck = action === 'check';
    const isChecked = await checkbox.isChecked();
