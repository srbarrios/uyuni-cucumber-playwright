...
def product
  ...
  _product_raw, code = get_target('server').run('rpm -q patterns-uyuni_server', checkErrors: false)
  if code.zero?
    $product = 'Uyuni'
    return 'Uyuni'
  end
  _product_raw, code = get_target('server').run('rpm -q patterns-suma_server', checkErrors: false)
  if code.zero?
...
def product_version
  product_raw, code = get_target('server').run('rpm -q patterns-uyuni_server', checkErrors: false)
  m = product_raw.match(/patterns-uyuni_server-(.*)-.*/)
  return m[1] if code.zero? && !m.nil?

  product_raw, code = get_target('server').run('rpm -q patterns-suma_server', checkErrors: false)
...
# reportResult is set to true
#
# Implementation works around https://bugs.ruby-lang.org/issues/15886
def repeat_until_timeout(timeout: DEFAULT_TIMEOUT, retries: nil, message: nil, reportResult: false, dontRaise: false)
  begin
    last_result = nil
    ...
    Timeout.timeout(timeout) do
      ...
      detail = format_detail(message, last_result, reportResult)
    ...
    $stdout.puts "Timeout after #{timeout} seconds (Timeout.timeout)#{format_detail(message, last_result, reportResult)}"
    raise e unless dontRaise
...
# @param reportResult [Boolean] Whether to include the last result in the formatted message.
# @return [String] The formatted detail message.
def format_detail(message, last_result, reportResult)
  formatted_message = "#{': ' unless message.nil?}#{message}"
  formatted_result = "#{', last result was: ' unless last_result.nil?}#{last_result}" if reportResult
...
def suse_host?(name, runsInContainer: true)
  node = get_target(name)
  osFamily = runsInContainer ? node.osFamily : node.local_osFamily
  %w[sles opensuse opensuse-leap sle-micro suse-microos opensuse-leap-micro].include? osFamily
end
...
def slemicro_host?(name, runsInContainer: true)
  node = get_target(name)
  osFamily = runsInContainer ? node.osFamily : node.local_osFamily
  (name.include? 'slemicro') || (name.include? 'micro') || osFamily.include?('sle-micro') || osFamily.include?('suse-microos') || osFamily.include?('sl-micro')
end
...
def leapmicro_host?(name, runsInContainer: true)
  node = get_target(name)
  osFamily = runsInContainer ? node.osFamily : node.local_osFamily
  osFamily.include?('opensuse-leap-micro')
end
...
def transactional_system?(name, runsInContainer: true)
  slemicro_host?(name, runsInContainer: runsInContainer) || leapmicro_host?(name, runsInContainer: runsInContainer)
end
...
def suse_proxy_transactional?
  ENV.key?(ENV_VAR_BY_HOST['proxy']) && transactional_system?('proxy', runsInContainer: false)
end
...
def rh_host?(name)
  osFamily = get_target(name).osFamily
  %w[alma almalinux amzn centos liberty ol oracle rocky redhat rhel].include? osFamily
end
...
def deb_host?(name)
  osFamily = get_target(name).osFamily
  %w[debian ubuntu].include? osFamily
end
...
def extract_logs_from_node(node, host)
  begin
    ...
    if osFamily.match?(/^opensuse/) && !$is_gh_validation && !transactional_system?(host)
    ...
    node.run('journalctl > /var/log/messages', checkErrors: false)
    node.run('venv-salt-call --local grains.items | tee -a /var/log/salt_grains', verbose: true, checkErrors: false) unless hostByNode[node] == 'server'
    node.run("tar cfvJP /tmp/#{node.fullHostname}-logs.tar.xz /var/log/ || [[ $? -eq 1 ]]")
    `mkdir logs` unless Dir.exist?('logs')
    success = file_extract(node, "/tmp/#{node.fullHostname}-logs.tar.xz", "logs/#{node.fullHostname}-logs.tar.xz")
...
def get_system_id(node)
  result = $api_test.system.search_by_name(node.fullHostname)
  raise "No system found for hostname: #{node.fullHostname}" unless result.any?
...
def check_restart(host, node, time_out)
  ...
  repeat_until_timeout(timeout: time_out, message: 'machine didn\'t come up') do
    _out, code = node.run('ls', checkErrors: false, timeout: 10)
    if code.zero?
...
def get_gpg_keys(node, target = get_target('server'))
  ...
  when /^sles/
    ...
    if osVersion.start_with?('15')
      osVersion = 12
    elsif osVersion.start_with?('12')
      # SLE12 GPG keys don't contain service pack strings
      osVersion = osVersion.slice(0, 2)
    end
    gpg_keys, _code = target.run("cd /srv/www/htdocs/pub/ && ls -1 sle#{osVersion}*", checkErrors: false)
  when /^centos/
    gpg_keys, _code = target.run("cd /srv/www/htdocs/pub/ && ls -1 #{osFamily}#{osVersion}* res*", checkErrors: false)
  else
    gpg_keys, _code = target.run("cd /srv/www/htdocs/pub/ && ls -1 #{osFamily}*", checkErrors: false)
  end
...
def update_controller_ca
  ...
  server_name = get_target('server').fullHostname
...
def channel_is_synced?(channel)
  ...
  result, code = get_target('server').run("dumpsolv /var/cache/rhn/repodata/#{channel}/solv", verbose: false, checkErrors: false)
  if code.zero? && !result.include?('repo size: 0')
    ...
    _result, new_code = get_target('server').run("dumpsolv /var/cache/rhn/repodata/#{channel}/solv.new", verbose: false, checkErrors: false)
    ...
    if EMPTY_CHANNELS.include?(channel)
      ...
      _result, code = get_target('server').run("zcat /var/cache/rhn/repodata/#{channel}/*primary.xml.gz | grep 'packages=\"0\"'", verbose: false, checkErrors: false)
    ...
    _result, new_code = get_target('server').run(command, verbose: false, checkErrors: false)
...
def new_api_client
  hostname = get_target('server').fullHostname
...
def server_secret
  rhnconf, _code = get_target('server').run('cat /etc/rhn/rhn.conf', checkErrors: false)
...
def trigger_upgrade(hostname, package)
  get_target('server').run("spacecmd -u admin -p admin system_upgradepackage #{hostname} #{package} -y", checkErrors: true)
end
