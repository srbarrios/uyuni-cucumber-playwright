...
export class HttpClient {
    ...
    constructor(host: string, sslVerify: boolean = true) {
        ...
        this.httpClient = axios.create({
            baseURL: `https://${host}`,
            timeout: TIMEOUTS.long * 1000, // Convert to milliseconds
    ...
    async call(name: string, params?: CallParams): Promise<any> {
        ...
        try {
            ...
            if (name === 'auth.login') {
                ...
                console.error('Failed to extract session cookie from auth.login response');
                return null;
            }

            // For all other calls, parse and return the result
            const jsonBody = response.data as ApiCallResult;
            expect(jsonBody.success, `API failure: ${jsonBody.message}`).toBeTruthy();
