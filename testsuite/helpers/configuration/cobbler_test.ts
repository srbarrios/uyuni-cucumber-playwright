...
export class CobblerTest {
  ...
  constructor() {
    ...
    // Cobbler API uses HTTP by default
  ...
  async get_list(what: 'systems' | 'profiles' | 'distros'): Promise<string[]> {
  ...
  async distro_create(name: string, kernel: string, initrd: string, breed: string = 'suse'): Promise<any> {
    try {
  ...
  async profile_create(name: string, distro: string, location: string): Promise<any> {
    try {
  ...
  async system_create(name: string, profile: string): Promise<any> {
    try {
  ...
  async system_remove(name: string): Promise<void> {
    if (!(await this.element_exists('systems', name))) {
  ...
  async element_exists(element_type: 'distros' | 'profiles' | 'systems' | 'repos', name: string): Promise<boolean> {
    return this.exists(element_type, 'name', name);
  }

  async repo_get_key(name: string, key: string): Promise<any> {
    if (await this.element_exists('repos', name)) {
  ...
  async profile_modify(name: string, attribute: string, value: any): Promise<any> {
    try {
  ...
  async distro_modify(name: string, attribute: string, value: any): Promise<any> {
    try {
  ...
  async system_modify(name: string, attribute: string, value: any): Promise<any> {
    try {
  ...
  async distro_remove(name: string): Promise<void> {
    if (!(await this.element_exists('distros', name))) {
  ...
  async profile_remove(name: string): Promise<void> {
    if (!(await this.element_exists('profiles', name))) {
  ...
  get_system_handle(name: string): Promise<any> {
    return this.methodCall('get_system_handle', [name, this.token]);
  }

  get_profile_handle(name: string): Promise<any> {
    return this.methodCall('get_profile_handle', [name, this.token]);
  }

  get_distro_handle(name: string): Promise<any> {
