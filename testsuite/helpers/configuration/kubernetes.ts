...
export async function generateCertificate(name: string, fqdn: string): Promise<[string, string, string]> {
    ...
    const {returnCode} = await server.runLocal(applyCmd);
    if (returnCode !== 0) {
    ...
    let rc = (await server.runLocal(`kubectl get secret uyuni-${name}-cert -o jsonpath='{.data.tls\\.crt}' | base64 -d >${crtPath}`)).returnCode;
    if (rc !== 0) throw new Error(`Failed to store ${name} certificate`);
    rc = (await server.runLocal(`kubectl get secret uyuni-${name}-cert -o jsonpath='{.data.tls\\.key}' | base64 -d >${keyPath}`)).returnCode;
