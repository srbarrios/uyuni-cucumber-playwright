...
import {globalVars} from '../core/env.js';

// Check if a file exists on the given node
export async function fileExists(node: RemoteNode, file: string): Promise<boolean> {
    return node.fileExists(file);
}

// Delete a file on the specified node
export async function fileDelete(node: RemoteNode, file: string): Promise<boolean> {
    return await node.fileDelete(file);
}
...
export async function folderExists(node: RemoteNode, folder: string): Promise<boolean> {
    return node.folderExists(folder);
}

// Delete a folder on the specified node
export async function folderDelete(node: RemoteNode, folder: string): Promise<boolean> {
    return node.folderDelete(folder);
}
...
export async function fileExtract(node: RemoteNode, remoteFile: string, localFile: string): Promise<boolean> {
    return await node.extract(remoteFile, localFile);
}
...
export async function fileInject(node: RemoteNode, localFile: string, remoteFile: string): Promise<boolean> {
    return await node.inject(localFile, remoteFile);
}
...
export async function injectSaltPillarFile(source: string, file: string): Promise<boolean> {
    ...
    const success = await fileInject(await server, source, dest);
    if (!success) throw new Error('File injection failed');
    await (await server).run(`chown -R salt:salt ${dest}`);
    return success;
}
...
export async function getVariableFromConfFile(host: string, filePath: string, variableName: string): Promise<string> {
    ...
    const {stdout, returnCode} = await node.run(`sed -n 's/^${variableName} = \\(.\\*\\)/\\1/p' < ${filePath}`);
    if (returnCode !== 0) throw new Error(`Reading ${variableName} from file on ${host} ${filePath} failed`);
    return stdout.trim();
}
...
export async function getChecksumPath(dir: string, originalFileName: string, fileUrl: string): Promise<string> {
    ...
    if (globalVars.mirror) {
        ...
        for (const name of checksumFileNames) {
            ...
            const {returnCode} = await server.run(`cd ${dir} && curl --insecure --fail ${checksumUrl} -o ${name}`, {
                runsInContainer: false,
                timeout: 10,
                checkErrors: false
            });
            if (returnCode === 0) return `${dir}/${name}`;
        }
