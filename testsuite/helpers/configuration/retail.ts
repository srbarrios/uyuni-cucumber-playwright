...
import {getApiTest, getProduct} from '../core/commonlib.js';
...
export async function computeKiwiProfileFilename(host: string): Promise<string> {
    ...
    const product = await getProduct(await getTarget('server') as RemoteNode);
    switch (image) {
        case 'sles15sp7o':
        case 'sles15sp4o':
...
export async function retrieveBuildHostId(): Promise<number> {
    ...
    if (!api?.system?.listSystems) throw new Error('API system namespace not available');
    const systems = await api.system.listSystems();
    if (!systems) throw new Error('Cannot list systems');
    const hostname = (await getTarget('build_host')).fullHostname;
