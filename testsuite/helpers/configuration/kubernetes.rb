...
def generate_certificate(name, fqdn)
  ...
  repeat_until_timeout(timeout: 600, message: "Kubernetes uyuni-#{name}-cert secret has not been defined") do
    _result, code = get_target('server').run_local("kubectl get secret uyuni-#{name}-cert", checkErrors: false)
    break if code.zero?
...
def running_k3s?
  _out, code = get_target('server').run_local('systemctl is-active k3s', checkErrors: false)
  code.zero?
end
