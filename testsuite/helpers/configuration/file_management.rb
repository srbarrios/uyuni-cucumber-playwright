...
def get_checksum_path(dir, original_file_name, file_url)
  ...
  if $mirror
    output, _code = server.run(cmd, runsInContainer: false)
    files = output.split("\n")
  ...
  # attempt to download the checksum file
    ...
    checksum_file_names.each do |name|
      ...
      _output, code = server.run("cd #{dir} && curl --insecure #{checksum_url} -o #{name}", runsInContainer: false, timeout: 10)
...
def validate_checksum_with_file(original_file_name, file_path, checksum_path)
  ...
  checksum_line, _code = get_target('server').run(cmd, runsInContainer: false)
...
def validate_checksum(file_path, expected_checksum)
  ...
  file_checksum, _code = get_target('server').run(cmd, runsInContainer: false)
